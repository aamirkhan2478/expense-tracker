...
const TEMPLATE_REGISTRY = {
  ...
  "budget-warning": {
    ...
    preview: (v) =>
      `You've used ${v.percentage}% of your ${v.category} budget.`,
  ...
  "upcoming-reminder": {
    ...
    preview: (v) =>
      `Your recurring payment ${v.transactionName} (${v.amount}) is due soon.`,
  ...
  "weekly-spending-summary": {
    ...
    preview: (v) =>
      `Spending: ${v.totalSpending} | Savings: ${v.netSavings} this week.`,
  ...
  "recurring-batch-summary": {
    ...
    preview: (v) =>
      `${v.totalSuccess} of ${v.totalProcessed} recurring transactions processed.`,
  ...
  "savings-milestone": {
    ...
    preview: (v) =>
      `You've reached ${v.progressPercent}% of your savings goal!`,
    preferenceKey: "savingsMilestone",
  },
  "bulk-import-summary": {
    template: bulkImportSummaryTemplate,
    subject: (v) =>
      `Import Complete: ${v.successCount} of ${v.totalProcessed} Transactions Imported`,
    preview: (v) =>
      `Your import is done. ${v.successCount} records imported successfully.`,
    ...
    if (userId) {
      ...
      if (user?.notificationPreferences) {
        if (user.notificationPreferences[preferenceKey] === false) {
          console.log(
            `[Email] User disabled template skipped: ${type} for user ${userId}`,
          );
          return false;
        }
      }
...
async function sendTemplatedEmail(to, type, variables, userId) {
  ...
  const subject =
    typeof registration.subject === "function"
      ? registration.subject(variables)
      : registration.subject;

  const previewText =
    typeof registration.preview === "function"
...
async function sendBudgetWarningEmail(
  to,
  name,
  category,
  spent,
...
async function sendBudgetExceededEmail(
  to,
  name,
  category,
  spent,
...
async function sendFailedLoginAlert(
  ...
  return sendTemplatedEmail(
    ...
    "failed-login",
    {
      ...
      ipAddress,
      userAgent,
      resetUrl: `${config.frontendUrl}/forget-password`,
    },
    userId,
  );
}
...
async function sendLoginNotificationEmail(
  to,
  name,
  { timestamp, device, browser, os, location, ipAddress, loginMethod },
  userId,
) {
  return sendTemplatedEmail(
    to,
    "login-notification",
    {
      ...
      secureAccountUrl: `${config.frontendUrl}/forget-password`,
    },
    userId,
  );
}

async function sendLargeExpenseAlertEmail(
  to,
  name,
  { amount, category, merchant, paymentMethod, date, budgetImpact },
  userId,
) {
  return sendTemplatedEmail(
    to,
    "large-expense-alert",
    {
      ...
      budgetImpact:
        budgetImpact ||
        "This expense has been recorded against your category budget.",
...
async function sendUpcomingReminderEmail(
  to,
  name,
  { transactionName, amount, category, dueDate, frequency, nextPaymentDate },
  userId,
) {
  return sendTemplatedEmail(
    to,
...
async function sendWeeklySpendingSummaryEmail(
  to,
  name,
  {
    weekRange,
  ...
  const topCategoriesHtml = Array.isArray(topCategories)
    ? topCategories
        .map((c, i) => `${i + 1}. <strong>${c.name}</strong> — ${c.amount}`)
        .join("<br/>")
  ...
  return sendTemplatedEmail(
    ...
    "weekly-spending-summary",
    {
      ...
      netSavings,
      budgetUsage:
        typeof budgetUsage === "number" ? budgetUsage.toFixed(0) : budgetUsage,
      topCategories: topCategoriesHtml,
      ...
      financialInsight:
        financialInsight ||
        "Keep tracking your spending to get personalized insights!",
...
async function sendRecurringBatchSummaryEmail(
  to,
  name,
  {
    totalProcessed,
...
async function sendOverspendingAlertEmail(
  to,
  name,
  {
    alertMessage,
  ...
  const actionsHtml = Array.isArray(suggestedActions)
    ? "<ul style='margin:0;padding-left:18px'>" +
      suggestedActions.map((a) => `<li>${a}</li>`).join("") +
      "</ul>"
...
async function sendSavingsMilestoneEmail(
  to,
  name,
  {
    milestoneName,
  ...
  return sendTemplatedEmail(
    ...
    "savings-milestone",
    {
      ...
      encouragingMessage:
        encouragingMessage ||
        "You're doing an incredible job managing your finances. Keep it up!",
...
async function sendBulkImportSummaryEmail(
  to,
  name,
  {
    totalProcessed,
  ...
  if (failedCount > 0 && Array.isArray(errors) && errors.length > 0) {
    errorSummaryHtml = `<strong>Import Errors:</strong><br/>${errors
      .slice(0, 10)
      .map((e) => `<span style="color:#DC2626">• ${e}</span>`)
      .join("<br/>")}`;
    if (errors.length > 10)
      errorSummaryHtml += `<br/><em>...and ${errors.length - 10} more. Download the full report for details.</em>`;
  }
